void Maze() {
  delay(500);
  while (1) {
    const int NUM_ITEMS_maze = 4;         // number of items in the list and also the number of screenshots and screenshots with QR codes (other screens)
    const int MAX_ITEM_LENGTH_maze = 20;  // maximum characters for the item name

    char menu_maze[NUM_ITEMS_maze][MAX_ITEM_LENGTH_maze] = { // array with item names
      { "Sensor Test" },
      { "Scan The Maze" },
      { "Solve The Maze" },
      { "Solution" }
    ...
    if (current_screen == 0) {  // MENU SCREEN
      ...
      if ((digitalRead(BUTTON_UP_PIN) == LOW) && (button_up_clicked == 0)) {  // up button clicked - jump to previous menu item
        ...
        if (item_selected < 0) {                                              // if first item was selected, jump to last item
          item_selected = NUM_ITEMS_maze - 1;
        }
      } else if ((digitalRead(BUTTON_DOWN_PIN) == LOW) && (button_down_clicked == 0)) {  // down button clicked - jump to next menu item
        item_selected = item_selected + 1;                                               // select next item
        button_down_clicked = 1;                                                         // set button to clicked to only perform the action once
        if (item_selected >= NUM_ITEMS_maze) {                                            // last item was selected, jump to first menu item
          item_selected = 0;
        }
      }
    ...
    if (item_sel_previous < 0) {
      item_sel_previous = NUM_ITEMS_maze - 1;  // previous item would be below first = make it the last
    }
    item_sel_next = item_selected + 1;
    if (item_sel_next >= NUM_ITEMS_maze) {
    ...
    if (item_selected == 0 && button_select_clicked == 1) {
      ...
      Solve_maze();
    } else if (item_selected == 3 && button_select_clicked == 1) {
      Solution();
    }
    ...
    do {
      if (current_screen == 0) {  // MENU SCREEN
        ...
        u8g.drawStr(10, 15, menu_maze[item_sel_previous]);

        // draw selected item as icon + label in bold font
        u8g.setFont(u8g_font_7x14B);
        u8g.drawStr(10, 15 + 20 + 2, menu_maze[item_selected]);

        // draw next item as icon + label
        u8g.setFont(u8g_font_profont12);
        u8g.drawStr(10, 15 + 20 + 20 + 2 + 2, menu_maze[item_sel_next]);

        // draw scrollbar background
        u8g.drawBitmapP(128 - 8, 0, 8 / 8, 64, bitmap_scrollbar_background);
        // draw scrollbar handle
        u8g.drawBox(125, 64 / NUM_ITEMS_maze * item_selected, 3, 64 / NUM_ITEMS_maze);
      }
...
void Scan_maze() {
  ...
  u8g.firstPage();
  do {
    u8g.drawStr(25, 20, "Scanning");  // Print the text at coordinates (10, 20)
  } while (u8g.nextPage());
...
void Solve_maze() {
  ...
  u8g.firstPage();
  do {
    u8g.drawStr(35, 20, "Solving");  // Print the text at coordinates (10, 20)
  } while (u8g.nextPage());
...
void Solution() {
  ...
  do {
    ...
    u8g.print("Maze Solution: ");  // Print the sensor value at coordinates (90, 40)
  }  while (u8g.nextPage());
